...
#hiding command-line parameters
$0 = "bybit-bot";
...
if (defined $config->{ARGV}->{'daemonize'}) {
    my $name          = "bybit-bot";
...
# Heartbeat
                ...
                if ((defined $heartbeat->{'err'} && $heartbeat->{'err'} == 1) || $sighup) {
